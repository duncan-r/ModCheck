...
either the Stage or Flow results. Changing the option in the dropdown box will result
in the validation being run against the chosen series.
...
Click on the "<" or ">" buttons to increment or decrement by a single timestep, 
respectively. Click to the left or right of the slider button to incremement by 20 
timesteps. Drag the slider to the position required for larger movement.

Bridges and spills will be added to the cross section viewer soon.
...
is applied to a smoothed version of the time series over a moving window to try and 
...
result in missing issues, so this is preferred. 
"""
...
MODEL_FILES_VARS_CHECK = """
Check the run parameters and files from a model delivery.


Search the contents of a given model directory to find files and check them against
those found within Flood Modeller and TUFLOW model and log files. Review the 
configuration of model run parameters and summarise diagnostics and warnings.

The run variables and summary allows you to review the 1D and 2D FMP and TUFLOW run
parameters, check them against the default values and interrogate some of the
diagnostic information from the simulation output files.

Once a model folder has been selected in the Model root folder dialog, it will be
searched to summarise the contents. A full list of all files found within the 
folder, listed by type, will be included in the All Files tab. These can be used
to cross reference any of the other results to check for accuracy.

Once loaded, all of the main model files found will be listed in the dropdown box
above the tabs. Flood Modeller models will be prefixed with "FM" and TUFLOW
models with "TUFLOW".
...
- The location of the file when the model was run (Original Path).
...
Variables tab:
The Variables tab contains a summary of all of all non-default run parameters used 
in the FM or TUFLOW model, what the default value is and a description explaining 
the parameter. 


Diagnostics tab:
The Diagnostics tab will load key information from the FM .ZZD diagnostics or 
TUFLOW .TLF file outputs. These include things like the run completion status, 
convergence issues, negative depths, etc. Checks, warnings, and errors found in the
diagnostics and log files will be shown in the bottom table. 


File Tree (Not currently implemented):
